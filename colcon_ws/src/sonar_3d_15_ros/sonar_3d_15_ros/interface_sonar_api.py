...
def get_speed(ip: str) -> int:
    ...
    """
    resp_speed = requests.get(
        f"http://{ip}/api/v1/integration/acoustics/speed_of_sound")
    try:
...
def get_acoustics(ip: str):
    ...
    """
    resp_acoustics = requests.get(
        f"http://{ip}/api/v1/integration/acoustics/enabled")
    try:
...
def describe_response(endpoint: str, response: requests.models.Response) -> None:
    """Print a more detailed response for the endpoint."""
    if response.ok:
